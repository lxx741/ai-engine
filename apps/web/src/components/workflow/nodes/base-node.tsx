    ...
    >
      ...
      {nodeData.status && nodeData.status !== 'default' && (
        <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-white border-2 border-white">
          {nodeData.status === 'running' && <div className="w-full h-full bg-blue-400 animate-pulse rounded-full" />}
          {nodeData.status === 'success' && <div className="w-full h-full bg-green-400 rounded-full" />}
